...
public class GetEmployeesServiceTest {
    ...
    public void test_delete_employee_when_all_is_ok() throws Exception {
        ...
        ReflectionTestUtils.setField(getEmployeesService, "bitacoraRepository", bitacoraRepository);

        Mockito.when(employeeRepository.findAll()).thenReturn(getEmployeesDTO());

        Mockito.when(bitacoraRepository.save(Mockito.any())).thenReturn(getBitacoraDTO());

        Mono<List<Employee>> response = getEmployeesService.getEmployees();
